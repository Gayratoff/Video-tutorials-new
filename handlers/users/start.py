...
async def bot_start(message: types.Message):
    ...
    await  message.answer(text=f"<b>🧑🏻‍💻 Botdagi obunachilar: {user_id} ta\n\nFaol obunachilar: {all_users} ta\nBotni tark etganlar: {blocked_users} ta\n\n</b>"
                               f"📊 <a href='{'https://t.me/ITSchoolUz_Robot'}'>IT School bot</a> <b>statistikasi</b> ",disable_web_page_preview=True,reply_markup=Menu_button)
...
async def bot_start(message: types.Message):
    ...
    for x in users:
        try:
            ...
            blocked_users += 1
    try:
        await  message.answer(text=f"<b>{all_users} ta foidalanuvchilarga reklama yuborildi ✅\n\n{blocked_users} ta odam botni bloklagan</b>")
    except Exception:
        pass
